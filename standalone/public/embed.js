...
(function () {
    ...
    /* --- Fix: ensure cards and images fill grid cells properly --- */
    .ai-lp-grid-2col > .grid__item,
    .ai-lp-grid-3col > .grid__item {
        width: 100% !important;
    ...
    .ai-lp-grid-2col .card,
    .ai-lp-grid-3col .card,
    .ai-lp-grid-2col .card__inner,
    .ai-lp-grid-3col .card__inner {
        width: 100% !important;
        max-width: 100% !important;
    }
    .ai-lp-grid-2col .card__media,
    .ai-lp-grid-3col .card__media {
        width: 100% !important;
    }
    ...
    /* --- Fix: product info (title/price) left padding in grid mode --- */
    .ai-lp-grid-2col .card__content,
    .ai-lp-grid-2col .card-information {
...
`;
    ...
    function showWelcomeModal() {
        ...
        const modalHTML = `
            ...
            <div class="ai-welcome-modal" id="ai-welcome-modal">
                ...
                <div class="ai-welcome-subtitle">Your Personalized Pet Store</div>
                <h2 class="ai-welcome-title">${titleText}</h2>
                <p class="ai-welcome-text">
                    Your entire experience is <strong>tailored to you</strong> — from page layout and product picks 
                    to AI-generated imagery, everything is personalized in real-time for you and your furry friend. 
                    Content may take a moment to appear — it's worth the wait!
                </p>
    ...
    // =====================
    // Landing Page 个性化引擎 (Theme-Based LLM)
    // =====================
    ...
    var THEMES = {
        ...
        2:  { name: 'Story Arc',
              sectionOrder: ['iwt','products','collections'], gridCols: null, featuredHero: false,
              cardShadow: false, cardBorder: false, hoverZoom: false, hoverLift: false, rounded: false,
              flipIWT: true, iwtOverlap: true, parallax: false, warmBg: false, spacing: 'spacious',
        ...
        3:  { name: 'Flash Deal',
              ...
              stickyVibe: true, dividers: false, promoBanner: true, socialProof: false, categoryPills: false },
        4:  { name: 'Gallery',
              sectionOrder: ['products','iwt','collections'], gridCols: null, featuredHero: false,
              cardShadow: false, cardBorder: false, hoverZoom: false, hoverLift: false, rounded: false,
              flipIWT: false, iwtOverlap: false, parallax: true, warmBg: false, spacing: 'spacious',
              stickyVibe: false, dividers: false, promoBanner: false, socialProof: false, categoryPills: true },
        5:  { name: 'Discovery',
              sectionOrder: ['collections','iwt','products'], gridCols: null, featuredHero: false,
              cardShadow: false, cardBorder: false, hoverZoom: false, hoverLift: false, rounded: false,
        ...
        7:  { name: 'Playful',
              sectionOrder: ['products','collections','iwt'], gridCols: null, featuredHero: false,
              cardShadow: false, cardBorder: false, hoverZoom: false, hoverLift: false, rounded: false,
              flipIWT: true, iwtOverlap: false, parallax: false, warmBg: false, spacing: null,
              stickyVibe: true, dividers: false, promoBanner: true, socialProof: false, categoryPills: false },
        8:  { name: 'Cozy',
              sectionOrder: ['iwt','products','collections'], gridCols: null, featuredHero: false,
              cardShadow: false, cardBorder: false, hoverZoom: false, hoverLift: false, rounded: false,
              flipIWT: false, iwtOverlap: false, parallax: false, warmBg: false, spacing: 'spacious',
              stickyVibe: false, dividers: true, promoBanner: false, socialProof: true, categoryPills: false },
        9:  { name: 'Impact',
              sectionOrder: ['products','iwt','collections'], gridCols: 2, featuredHero: true,
    ...
    function preloadPersonalization() {
        ...
        // 等 productMeta 准备好再发请求（需要把产品列表发给 LLM）
        _personalizationPromise = (async function () {
            var _llmFetchStart = Date.now();
            log('[AI-LP] ⏱️ Personalization: waiting for product meta...');
            ...
            log('[AI-LP] ⏱️ Personalization fetch started at', new Date().toISOString(), 'with', products.length, 'products');
            ...
            try {
                ...
                log('[AI-LP] ⏱️ Personalization fetch response:', res.status, 'in', elapsed1 + 'ms');
                ...
                if (data && data.success && data.config) {
                    ...
                    log('[AI-LP] ⏱️ LLM personalization loaded in', elapsed2 + 'ms', data.cached ? '(server-cached)' : '(fresh)', 'serverTime:', data.processingTime + 'ms');
                    log('[AI-LP] Config:', JSON.stringify(data.config, null, 2));
                    return data.config;
                }
                return null;
            } catch (err) {
                log('[AI-LP] ⏱️ Personalization fetch FAILED in', (Date.now() - _llmFetchStart) + 'ms:', err && err.message ? err.message : err);
                return null;
            }
        })();
    ...
    function reorderSections(sectionOrder) {
        try {
            ...
            log('[AI-LP] Sections reordered:', sectionOrder.join(' → '));
        } catch (err) {
    ...
    function setTextSafe(section, selector, text) {
        ...
        log('[AI-LP] Text: "' + (innerEl.dataset.aiOriginalText || '').slice(0, 20) + '..." → "' + text + '"');
    }
    ...
    function applyTheme(themeNum, config, container) {
        ...
        if (container) {
            ...
            // Fix image sizes attribute — Dawn's srcset uses sizes based on slider widths,
            // which causes browsers to load tiny images when we switch to grid.
            // Only update when gridCols is set (2-col); default layout keeps original sizes.
            if (theme.gridCols === 2) {
                var newSizes = '(min-width: 750px) calc(50vw - 4rem), calc(100vw - 2rem)';
                container.querySelectorAll('img[sizes]').forEach(function (img) {
                    img.setAttribute('sizes', newSizes);
                });
                log('[AI-LP] Updated img sizes for 2-col grid');
            }
        }
    ...
    async function applyLandingPersonalization() {
        try {
            ...
            if (llmConfig) { log('[AI-LP] ⏱️ Phase 2: config in cache (0ms)'); }
            if (!llmConfig) {
                log('[AI-LP] ⏱️ Phase 2: Waiting for LLM (max 10s)...');
                ...
                log('[AI-LP] ⏱️ Phase 2: wait done in', (Date.now() - _phase2Start) + 'ms', llmConfig ? '(got config)' : '(timeout)');
            }
            ...
            log('[AI-LP] ✅ Done' + (llmConfig ? ' (theme ' + llmConfig.theme + ')' : ' (fallback)'));
        } catch (err) {
            log('[AI-LP] ❌ Error:', err);
