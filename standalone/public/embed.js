...
`;
    ...
    function applyTheme(themeNum, config, container) {
        ...
        if (container) {
            ...
            // Update sizes to match actual grid column widths.
            var newSizes = theme.gridCols === 2
                ? '(min-width: 750px) calc(50vw - 4rem), calc(100vw - 2rem)'
                : '(min-width: 990px) calc(33vw - 3rem), (min-width: 750px) calc(50vw - 3rem), calc(100vw - 2rem)';
            container.querySelectorAll('img[sizes]').forEach(function (img) {
                img.setAttribute('sizes', newSizes);
            });
            log('[AI-LP] Updated img sizes for', theme.gridCols + '-col grid');
        }
